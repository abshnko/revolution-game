...
const AddOptions = ({
  ...
  const handleSubmit = () => {
    let newArr = questionInitialState.options;
    if (!optionToUpdate) {
      let biggestID = 10;
      if (questionInitialState.options.length > 0) {
      ...
      setQuestionInitialState({ ...questionInitialState, options: newArr });
    } else {
  ...
  const updateOptionNext = (e) => {
    setOption({ ...option, next: parseInt(e.target.value) });
    console.log(option);
  };
            ...
            <label htmlFor="">
              ...
              >
                ...
                {questions.map((question) => {
                  return (
                    <option value={parseInt(question.id)}>{`${question.id} - ${
                      ...
                      <div className="buttons">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setOption(newOption);
                            const newOptions = questionInitialState.options.map(
                              (x) => (x.id === newOption.id ? newOption : x)
                            );
                            setQuestionInitialState({
                              ...questionInitialState,
                              options: newOptions,
